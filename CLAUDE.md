...
| `/implement` | **deletes** the marker first, then implements exactly one task via a difficulty-graded executor (S/M → subagents, L → main conversation); stamps the spec `in-progress`/`done` |
