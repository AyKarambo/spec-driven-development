...
6. Present the drafted spec for review (**gate**) — do **not** write code and do **not** create the issue yet. **Only after I approve:**
   ...
   - **Create the spec issue:** write the body to a transient, non-`.md` temp file (`.claude/sdd/issue-body.txt` — gitignored and gate-allowed) and run `gh issue create --title "[SDD] <slug>: <short title>" --body-file .claude/sdd/issue-body.txt --label sdd --label sdd:draft`, then **delete the temp file**. Report the created issue number and URL.
