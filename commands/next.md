...
1. Determine the current state (same logic as `/status`): whether `specs/<slug>.spec.md` exists and is non-empty, whether `specs/<slug>.plan.md` / `.tasks.md` exist, how many tasks are checked, and whether `.claude/sdd/phase` is set. (Resolve the slug from `$ARGUMENTS`, else the current branch.)
