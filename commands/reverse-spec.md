...
5. Present the spec for review (**gate**) — do **not** change any code and do **not** create the issue yet. **Only after I approve:**
   ...
   - **Create the issue as already-shipped:** write the body to a transient, gitignored temp file (`.claude/sdd/issue-body.md`) and run `gh issue create --title "[SDD] <slug>: <short title>" --body-file .claude/sdd/issue-body.md --label sdd --label sdd:done`, **delete the temp file**, then **close it** (`gh issue close <n> --reason completed`) — reverse-spec documents behavior that already shipped, so it starts `done`.
