...
**The issue is the only store for spec/plan/tasks.** You write real feature code here — but the spec, plan, and task checklist live in the GitHub issue, never in a repo file. Don't mirror or "save" them into `spec.md`/`plan.md`/`tasks.md`/a `specs/` folder; update the checklist in the issue via `gh`. Any issue-body text you pipe into `gh` goes through a **transient** non-`.md` file under `.claude/sdd/`, deleted right after.
...
6. Afterwards, **verify the result yourself** regardless of who executed it — run the tests and read the diff against the task's **Check**; a subagent saying "done" is not verification. Then briefly summarize the diff, and update the issue:
   - Flip that task's `- [ ]` to `- [x]` within the `## Tasks` section of the body (leave the `## Spec`/`## Technical Plan` content and all marker pairs untouched), write the new body to a transient non-`.md` file (`.claude/sdd/issue-body.txt`), run `gh issue edit <n> --body-file .claude/sdd/issue-body.txt`, then **delete the temp file**.
