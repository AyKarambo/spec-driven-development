...
1. Gather state:
   ...
   - Scan `specs/` for the file artifacts per feature: `<slug>.spec.md` (spec), `<slug>.plan.md` (plan), `<slug>.tasks.md` (tasks).
   - For the feature in focus (`$ARGUMENTS`), the active marker's slug, or each slug found among the spec/plan/tasks files, check whether `<slug>.spec.md` exists and is non-empty.
2. For each feature, report a compact line:
   `<slug>:  spec ✓  plan ✓  tasks ✓  impl ▸ N/M  → next: <step>`
