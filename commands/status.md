...
3. If `.claude/sdd/phase` is set, call out the **active gate** (e.g. `spec:invoice-export`) and that on-disk writes are restricted until `/implement` runs or the marker is deleted: **Markdown** may only go to the rule files `CLAUDE.md`/`AGENTS.md`/`.claude/rules/**` (or the gitignored `.claude/sdd/` scratch), so a spec/plan/tasks doc can't be saved as a committed `.md`; other feature code is limited to `CLAUDE.md`/`AGENTS.md`/`.claude/**`. Spec artifacts live in issues and are written via `gh`, so they're unaffected.
