---
description: Autopilot — after spec approval, run plan → tasks → implementation → review → PR autonomously
...
2. **Require an approved spec:** `specs/<slug>.spec.md` must exist and be non-empty — `/spec` only writes that file after approval, so its existence *is* the approval. If it's missing or empty, **stop** and point to `/spec`. Autopilot never writes or approves a spec itself.
3. Read the spec. If its **Open Questions** contain anything that materially changes what to build, **stop and ask** those questions first — don't guess intent autonomously.
...
Autopilot can be entered at **any point after spec approval** — right after `/spec`, mid-planning, mid-implementation, or with all tasks already done. Determine the current state (same logic as `/status`): does `specs/<slug>.plan.md` exist, does `.tasks.md` exist, how many tasks are checked, what does the spec's frontmatter `status:` say? Then run **only the missing phases, in order**. Each phase follows its own command's instructions — including its gate-marker behavior — with exactly one difference: **instead of stopping for approval, print the phase result compactly and continue.**

- **No plan** → run `/techplan`'s instructions (marker `plan:<slug>`, parallel `Explore` research, write `specs/<slug>.plan.md`, stamp `status: planned`) — then continue.
- **No tasks** → run `/breakdown`'s instructions (marker `tasks:<slug>`, write the checklist, run the spec↔tasks consistency check). If the check finds coverage gaps, fix the task list until every acceptance criterion is covered. If it surfaces a genuine contradiction in the spec, **stop** — that's a `/revise` decision, not yours.
- **Unchecked tasks** → the implementation loop below.
...
2. Give every subagent a **self-contained prompt**: the task text (Goal/Files/Check), the acceptance criteria it serves, the relevant plan excerpt, the project conventions (CLAUDE.md / AGENTS.md), and the instruction to write/update tests.
...
4. On success: tick the task in `…tasks.md`, advance the spec lifecycle exactly as `/implement` does (first checked task → `status: in-progress`; last → `status: done`, refresh `updated:`), and **commit** — one focused commit per task, so the PR reads as a reviewable series.
5. **Stop rules** — autopilot stops and reports instead of pushing through when:
   ...
   When stopping: report honestly what is done and committed, what failed, and why. A later `/auto` (or `/implement`) resumes from the checklist.
...
2. **Final self-check:** every acceptance criterion met, tests green, task list fully checked, spec stamped `done`.
3. **PR:** push the branch and open the pull request with `gh` — title from the spec's intent; body with a summary, a link to `specs/<slug>.spec.md`, the acceptance criteria as a checklist, and the review outcome. If a PR for this branch already exists, push and update it instead of creating a duplicate. No remote or no `gh`? Stop and report — the branch is ready locally.
