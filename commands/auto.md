...
- **No task sub-issues** → run `/breakdown`'s instructions (marker `tasks:<slug>`, create one task sub-issue per task with `--parent <spec#>`, run the spec↔tasks consistency check). If the check finds coverage gaps, fix the task list until every acceptance criterion is covered. If it surfaces a genuine contradiction in the spec, **stop** — that's a `/revise` decision, not yours.
- **Open task sub-issues** → the implementation loop below.
- **All sub-issues already closed** → skip straight to review & PR.

## Implementation loop — sub-agents graded by difficulty

As the first implementation action, **delete `.claude/sdd/phase`** — same contract as `/implement`: this is the moment feature code may be written. Then work through the **open** task sub-issues **in order** (the order encodes dependencies):
...
2. Give every subagent a **self-contained prompt**: the task text (Goal/Files/Check from the task sub-issue), the acceptance criteria it serves, the relevant `## Technical Plan` excerpt, the project conventions (CLAUDE.md / AGENTS.md), and the instruction to write/update tests.
...
4. On success: **close the task sub-issue** (`gh issue close <task#> --reason completed --comment "<summary>"`), advance the spec's status label exactly as `/implement` does (first closed task → add `sdd:in-progress`/remove `sdd:planned`; last → add `sdd:done`, remove the other `sdd:*` status label, and close the spec issue), and **commit** — one focused commit per task, so the PR reads as a reviewable series.
5. **Stop rules** — autopilot stops and reports instead of pushing through when:
   ...
   When stopping: report honestly what is done and committed, what failed, and why. A later `/auto` (or `/implement`) resumes from the still-open task sub-issues.
...
2. **Final self-check:** every acceptance criterion met, tests green, all task sub-issues closed, spec issue labeled `sdd:done` and closed.
