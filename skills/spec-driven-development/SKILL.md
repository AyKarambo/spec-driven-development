...
5. Each spec carries a `status:` (`draft → planned → in-progress → done`) the commands keep current. Once a feature is `done`, suggest **`/spec-cleanup`** to archive its now-throwaway plan/tasks (the spec is kept) so `specs/` doesn't fill with stale scaffolding.
6. The **merge stays a human decision** — AI review (`/code-review`, `/review`) informs, it does not auto-gate.
